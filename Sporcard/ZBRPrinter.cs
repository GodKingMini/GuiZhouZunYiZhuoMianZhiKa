...
namespace Sporcard
{
    public class ZBRPrinter
    {
        ...
        public static extern int ZBRPRNGetPrintStatus(IntPtr hPrinter, int printerType);
        ...
        public int GetDeviceSerial(out byte[] strDeviceSerial, out int err)
        {
            err = -1;
            int respSize = 0;
            strDeviceSerial = null;
            System.Text.ASCIIEncoding ascii = new System.Text.ASCIIEncoding();
            Byte[] drvName = null;
            LogHelper.WriteLog(typeof(FormMain), "GetDeviceSerial:" + drvName);
            return ZBRPRNGetPrinterSerialNumb((System.IntPtr)_handle, _prnType, strDeviceSerial, out respSize, out err);
        }
        ...
        public bool IsPrinterInErrorMode(out int errValue)
        {
            ...
            //ZBRPRNGetPrinterStatus(out errValue);
            string printName = "Zebra ZXP Series 3C USB Card Printer";
            errValue = 0;
            System.Text.ASCIIEncoding ascii = new System.Text.ASCIIEncoding();
            Byte[] drvName = null;
            drvName = ascii.GetBytes(printName);

            ZBRGetHandle(out _handle, drvName, out _prnType, out errValue);
            errValue = ZBRPRNGetPrintStatus(_handle, _prnType);
            if (errValue == 0)
                return false;
            return true;
